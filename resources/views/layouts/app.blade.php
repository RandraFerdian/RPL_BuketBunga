                    ...
                    <div class="hidden space-x-2 sm:ms-10 sm:flex items-center">
                        @auth
                            @if(Auth::user()->role === 'admin')
                                ...
                                <a href="{{ route('admin.kategori.index') }}" class="{{ request()->routeIs('admin.kategori.*') ? 'bg-pink-500 text-white shadow-sm' : 'text-gray-600 hover:text-pink-500' }} px-4 py-2 rounded-full text-sm font-medium transition-colors">Kategori</a>
                            @else
